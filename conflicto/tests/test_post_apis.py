...
class TestPostApis(TestCase):
    ...
    def test_post_retrieval(self):
        ...
        for p in Post.objects.all():
            Reaction.objects.create(user_id=self.user.id, object_uuid=p.uuid, object_type='POST',
    ...
    def test_post_action_2_times(self):
        ...
        self.assertNotIn('LIKE',
                         Reaction.objects.filter(object_uuid=post_uuid, object_type=Objects.POST).first().actions)
        self.assertEqual(
            Reaction.objects.filter(object_uuid=post_uuid, object_type=Objects.POST).first().actions.__len__(), 0)
